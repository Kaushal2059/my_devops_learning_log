# ☁️ AWS ECS Fargate Deployment — Housing Regression Project
...
```
The "ECS Task" choice matters — it sets up the **trust policy** controlling who can assume the role. Picking this means only ECS tasks can assume it, not EC2 instances or Lambda.
```
...
- **`AdministratorAccess` on `kaushal-cli`/`Rishav2`** is intentionally broad for this learning project but should eventually be scoped down to only the specific ECS/ECR/S3/IAM/ELB actions actually used, especially before this same user/key pair is reused in a CI/CD pipeline.
...
## Related entries in this log
- [`manual-hosting-django-project.md`](./manual-hosting-django-project.md) — the EC2/Nginx/Gunicorn deployment this built on conceptually, before moving to managed container orchestration
- [`DOCKER_AND_DOCKER_COMPOSE.md`](./DOCKER_AND_DOCKER_COMPOSE.md) — containerization fundamentals used to build both images here
- `CICD_GITHUB_ACTIONS.md` — will be extended with the GitHub Actions pipeline for this project once written up

**Project repo:** [`housing_regression_MLE`](https://github.com/rishav-sah/housing_regression_MLE)
